...
from flask_server.lock_manager import Lock_Manager
from flask_server.util import Util
...
class Noise_Detector(threading.Thread):
    def __init__(self):
        ...
        self.CHUNKS_PER_SEC = math.floor(
            self.RATE / self.CHUNK_SIZE)  # How many chunks make a second? (16.000 bytes/s, each chunk is 1.024 bytes, so 1s is 15 chunks)
        self.CHANNELS = 1
